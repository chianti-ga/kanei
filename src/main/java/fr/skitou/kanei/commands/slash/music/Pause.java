...
public class Pause implements ISlashCommand {
    ...
    public void onCommandReceived(SlashCommandInteractionEvent event) {
        ...
        if (guildMusic.player.isPaused()) {
            event.getHook().sendMessage(BotInstance.getBundleFromGuild(event.getGuild()).getString("music.resumed")).queue();
            guildMusic.player.setPaused(false);
        } else {
            event.getHook().sendMessage(BotInstance.getBundleFromGuild(event.getGuild()).getString("music.paused")).queue();
