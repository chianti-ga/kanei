...
public class Loop implements ISlashCommand {
    ...
    public void onCommandReceived(SlashCommandInteractionEvent event) {
        ...
        if (event.getOption("loop").getAsBoolean()) {
            event.getHook().sendMessage(BotInstance.getBundleFromGuild(event.getGuild()).getString("music.loopon")).queue();
        } else {
            event.getHook().sendMessage(BotInstance.getBundleFromGuild(event.getGuild()).getString("music.loopoff")).queue();
        }
    }
